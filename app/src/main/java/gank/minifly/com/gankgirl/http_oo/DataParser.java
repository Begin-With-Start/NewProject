...
public class DataParser {
	 ...
	 *            Bean
	 */
	public static void parseJSONObject(JSONObject jsonObject, Object bean)
			throws Exception {
		...
		for (Field field : fieldList) {
			...
			if (jsonObject.has(fieldName)
					...
					&& !"null"
							.equals(String.valueOf(jsonObject.get(fieldName)))
					&& String.valueOf(jsonObject.get(fieldName)).length() > 0) {

				String typeName = field.getType().getSimpleName();

				if ("List".equals(typeName) || "LinkedList".equals(typeName)) {
					...
					for (int j = 0; j < jsonArray.length(); j++) {
						...
						if (isInnerClass) {
							childBean = clz.getDeclaredConstructors()[0]
									.newInstance(bean);
						} else {
				...
				} else if ("ArrayList".equals(typeName)) {
					...
					field.set(bean, Integer.parseInt(String.valueOf(jsonObject
							.get(fieldName))));

				} else if ("float".equals(typeName) || "Float".equals(typeName)) {
					field.set(bean, Float.parseFloat(String.valueOf(jsonObject
							.get(fieldName))));

				} else if ("double".equals(typeName)
						|| "Double".equals(typeName)) {
					field.set(bean, Double.parseDouble(String
							.valueOf(jsonObject.get(fieldName))));

				} else if ("boolean".equals(typeName)
						|| "Boolean".equals(typeName)) {
					field.set(bean, Boolean.parseBoolean(String
							.valueOf(jsonObject.get(fieldName))));

				} else if ("long".equals(typeName) || "Long".equals(typeName)) {
					field.set(bean, Long.parseLong(String.valueOf(jsonObject
							.get(fieldName))));
